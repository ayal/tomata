...
/***/ function(module, exports, __webpack_require__) {
	...
	var Toma = _react2.default.createClass({
				...
				getInitialState: function getInitialState() {
							return { ruleText: '' + this.props.rule, rule: this.props.rule, firstrow: this.props.rrow() };
				},
				...
				calcnext: function calcnext(row) {
							...
							for (var i = 0; i < row.length; i++) {
										...
										var np = parseInt(this.state.rule.toString(2)[num] || 0);
										next[i + 1] = np;
							}
							return next;
				},
				...
				setRule: function setRule() {
							this.context.clearRect(0, 0, this.props.w * this.props.scale, this.props.w * this.props.scale);
							this.setState({ rule: parseInt(this.refs.rule.value) });
				},
				rrule: function rrule() {
							var newrule = _.random(rules);
							this.setState({ ruleText: '' + newrule, rule: newrule });
				},
				rrow: function rrow() {
							this.setState({ firstrow: this.rrow() });
				},
	...
	var App = _react2.default.createClass({
				...
				render: function render() {
							return _react2.default.createElement(Toma, { w: w, scale: scale, rule: rule, rrow: this.rrow });
				}
	});
